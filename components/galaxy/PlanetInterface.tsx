...
export default function PlanetInterface() {
  // 1. 核心修改：从 store 中解构出 openDeepDive
  const { focusedPlanet, enterOrbitView, galaxyNodes, openDeepDive } = useAppStore();
    ...
    <div className="w-full h-screen bg-[#080810] flex flex-col relative overflow-hidden">
      ...
      <div className="px-8 mb-6 flex gap-8 z-10">
        <div>
          <div className="text-3xl font-black text-white">
            {(realUserInsights?.length || 0) + (currentPlanetMock?.mockInsights?.length || 0)}
          </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-[120px] z-10 no-scrollbar space-y-4">
        ...
        {realUserInsights.map((node) => (
          <button 
            key={node.id} 
            onClick={() => openDeepDive(node)} // 绑定点击事件进入详情
            className="w-full text-left bg-white/10 border border-spark-blue/50 rounded-2xl p-5 animate-in slide-in-from-bottom-5 duration-500 active:scale-[0.98] transition-all"
          >
        ...
        {currentPlanetMock.mockInsights.map((item) => (
          <button 
            key={item.id} 
            onClick={() => openDeepDive(item)} // 绑定点击事件，确保 Mock 数据也能点击
            className="w-full text-left bg-white/5 border border-white/5 rounded-2xl p-5 opacity-80 hover:bg-white/[0.08] transition-all active:scale-[0.98]"
          >
             <div className="flex justify-between items-center mb-2">
